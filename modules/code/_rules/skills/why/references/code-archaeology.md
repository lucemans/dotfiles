...
## When the target looks defensive

Null checks, retry logic, timeout handling, rate limiting, feature flags, egress guards, and OOM handlers usually exist because something went wrong once. Hunt for that event specifically:

```bash
# Commits whose message admits an incident
git log --oneline --grep='incident\|outage\|hotfix\|regression\|postmortem\|revert' -- <file>

# A revert followed by a re-apply is a strong signal: the first attempt broke something
git log --oneline --grep='revert' --all

# The commit that introduced the exact guard, not the commit that last touched it
git log -S '<the guard expression>' --oneline -- <file>
```

```bash
# Issues and PRs that mention the symptom rather than the fix
gh issue list --state all --search '<error string or symptom>'
gh pr list --state all --search '<error string or symptom>'
```

An issue whose report predates the guard, and whose close references the PR that added it, is the strongest available evidence for defensive code. Fetch the full thread, not the title.
