...
Build each investigator's prompt from this template; fill in the placeholders. Append `code-archaeology.md`, which holds the query vocabulary every investigator works from.
...
The whole historical record here is the repository: commit history, PR and issue discussion, in-repo prose, and tests. Other investigators work different angles on that same record in parallel. Don't try to cover everything. Go deep on your assigned angle.
...
3. **Follow links within your assigned angle.** If a PR references another PR or commit, pull it. If an issue links a parent or sibling, pull it. If a doc links another doc, pull it. When a lead clearly belongs to another angle, record it under "Additional Leads" rather than chasing it yourself. Overlap between angles is expected and fine; wholesale duplication is waste.
...
### Angle
Which angle you investigated (commit and diff history, PR and issue discussion, in-repo prose and tests, pattern origin, and so on).
...
### Gaps
What you searched for and didn't find. Be specific: "Ran `git log -S 'MAX_RETRIES' --all`. Three commits, none with a message explaining the value." These absences are valuable data.

Separate two kinds of absence. A search that ran and returned nothing is evidence. A record that does not exist in this environment at all, such as a design doc that lives outside the repository or a conversation that happened off the record, is a boundary of the investigation. Say which one you hit.

### Additional Leads
Anything that suggests further investigation in a different angle, or outside the repository entirely. A PR that says "as discussed" without saying where, an author who would know, a linked URL that isn't reachable from here. Name it so the synthesizer can report it as an open thread rather than a dead end.
